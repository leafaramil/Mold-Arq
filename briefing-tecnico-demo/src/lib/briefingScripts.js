  ...
  topics: [
    {
      ...
      permiteAnexo: true,
      objetivo:
        'Pergunta 1: "Esse projeto é uma reforma no espaço que vocês já ocupam hoje, ou uma mudança para um endereço novo?" — opcoes: ["Reforma no espaço que já ocupamos hoje", "Mudança para um espaço novo"]. A resposta define a "regra do imóvel" usada em vários temas seguintes: só pergunte algo no presente sobre "hoje" (wifi de hoje, temperatura de hoje, equipamento a reaproveitar) se a empresa ocupa o espaço atual; se for mudança para endereço novo, pule essas perguntas sem comentário. Pergunta 2, mensagem separada e com a redação adaptada à resposta da pergunta 1: se for reforma no espaço atual, pergunte "Vocês têm o as-built (planta como construído) do escritório atual?"; se for mudança para endereço novo, pergunte "Vocês têm as plantas técnicas do novo espaço?" — em ambos os casos, opcoes: ["Sim", "Não"]. Se a resposta for sim, pergunta 3, sem opções: "Pode anexar todos os arquivos de uma vez, pelo botão de anexo — selecione todos juntos no seletor, porque depois de enviar não dá pra abrir o seletor de novo nesse tema." Não pergunte quais plantas especificamente são (arquitetura, elétrica etc.) — é só anexar o que tiver.',
    },
    {
      id: 2,
      rotulo: 'Perfil das estações de trabalho',
      objetivo:
        'Pergunta 1, sem opções: "Os postos de trabalho, de forma geral, usam desktop ou notebook? Têm monitores extras?". Pergunta 2, mensagem separada, sem opções: "A diretoria, a gerência ou alguma área específica tem uma configuração diferente dessa? Se sim, qual?". Pergunta 3, mensagem separada, sem opções: a partir SOMENTE do que foi mencionado nas respostas anteriores (nunca presuma itens que a pessoa não citou, como dock ou monitor extra, se ela não falou disso), proponha uma quantidade preliminar de tomadas por posto de trabalho — ex.: "Nesse caso, podemos considerar [lista dos itens realmente citados] tomadas por posto, mais uma de reserva. Faz sentido, ou preferem outra quantidade?" — deixe a pessoa responder livremente (confirmando ou já corrigindo o número na mesma resposta), sem oferecer botões de "confirmar" ou "ajustar" que a obriguem a uma segunda pergunta só para digitar o número. Não é preciso perguntar de novo sobre mesas com necessidade diferente — isso já foi coberto na pergunta 2.',
    },
    {
      id: 3,
