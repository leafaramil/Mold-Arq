...
// Uso local: preencha cotacao/.env.local com a POSTGRES_URL da Vercel e rode `npm run db:migrate`.
...
async function main() {
  ...
  if (!connectionString) {
    console.error("DATABASE_URL / POSTGRES_URL não definida. Configure cotacao/.env.local ou exporte a variável.");
    process.exit(1);
  }
