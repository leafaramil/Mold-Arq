...
// Trava local por biometria (WebAuthn/Passkey) — mesmo padrão do projeto
// Caderno (ver src/lib/biometria.ts de lá). Não é autenticação num servidor
// — o app não tem níveis de permissão, Rafael e Letícia veem tudo igual. É
// só um cadeado local: cada aparelho guarda sua própria credencial (numa
// chave privada que nunca sai do aparelho) e pede a digital/rosto pra abrir
// o app, em vez de só tocar num nome. Por isso não há verificação de
// assinatura no servidor — a garantia de segurança já vem inteira do
// sistema operacional.
